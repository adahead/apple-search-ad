...
class ReportingRequest extends ApiRequest
{
    ...
    public function setGranularity($granularity) {
        $possible = [
            static::GRANULARITY_DAILY,
            static::GRANULARITY_HOURLY,
            static::GRANULARITY_MONTHLY,
            static::GRANULARITY_WEEKLY
        ];
    ...
    {
        $possible = ['countryOrRegion', 'adminArea', 'deviceClass', 'ageRange', 'locality', 'gender'];
    ...
    /**
     * POST /v2/reports/campaigns
     * Get reports on campaigns within a specific org.
     * @throws \Exception
     */
    public function queryCampaignsReports() {
        $this->checkRequired();
        $this->setRequestType(static::REQUEST_MODE_READ)
            ->setPost()
            ->setUrl("reports/campaigns")
            ->setBody($this->compileRequestBody())
            ->run();
    }

    /**
     * POST /v2/reports/campaigns/<CAMPAIGN_ID>/adgroups
    ...
    public function queryReportsOnAdGroup($campaignId) {
        ...
        $this->setRequestType(static::REQUEST_MODE_READ)
            ->setPost()
            ->setUrl("reports/campaigns/" . $campaignId . "/adgroups")
            ->setBody($this->compileRequestBody())
            ->run();
    }

    /**
     * POST /v2/reports/campaigns/{campaignId}/creativesets
    ...
    public function queryReportsOnCreativeSet($campaignId)
    {
        ...
        $this->setRequestType(static::REQUEST_MODE_READ)
            ->setPost()
            ->setUrl("reports/campaigns/" . $campaignId . "/creativesets")
            ->setBody($this->compileRequestBody())
            ->run();
    }

    /**
     * POST /v2/reports/campaigns/<CAMPAIGN_ID>/keywords
     * Get reports on targeted keywords within a specific campaign.
     * @param $campaignId
     * @throws \Exception
     */
    public function queryReportsTargetedKeywords($campaignId) {
        ...
        $this->setRequestType(static::REQUEST_MODE_READ)
            ->setPost()
            ->setUrl("reports/campaigns/" . $campaignId . "/keywords")
            ->setBody($this->compileRequestBody())
            ->run();
    }

    /**
     * POST /v2/reports/campaigns/<CAMPAIGN_ID>/searchterms
     * Get reports on search terms for a specific campaign
     * @param $campaignId
     * @throws \Exception
     */
    public function queryReportsSearchTerm($campaignId) {
        ...
        $this->setRequestType(static::REQUEST_MODE_READ)
            ->setPost()
            ->setUrl("reports/campaigns/" . $campaignId . "/searchterms")
            ->setBody($this->compileRequestBody())
            ->run();
    }
}
